...
ram_rate=$(free --mega | awk '$1 == "Mem:" {print $3/$2 * 100}')

# DISK
disk_total=$(df -m | grep "/dev/" | grep -v "/boot" | awk '{disk_t += $2} END {printf ("%.1f\n"), disk_t/1024}')
...
# TCP CONNECTIONS
cpu_connection=$(ss -ta | grep "ESTAB" | wc -l)
...
printf "#CPU load: $cpu_fin%%\n";
printf "#Last boot: $last_boot\n";
printf "#LVM use: $use_lvm\n";
printf "#Connections TCP: $cpu_connection\n";
